...
def norm(train, test):
    ...
    normalizer = MinMaxScaler(feature_range=(0, 1)).fit(
        train
...
def main():
    ...
    train["attack"] = train.pop("Normal/Attack").map({"Attack": 1, "Normal": 0})
    test["attack"] = test.pop("Normal/Attack").map({"Attack": 1, "Normal": 0})
